  ...
  if (options.strategy === 'daemon') {
    ...
    ipc.on('message', function (msg) {
      ...
      helpers.processWatcher(msg.daemon, function () {
        ...
        // the extra argument will:
        // 1. let the the daemon.js know that it was restarted
        // 2. wait with spawning a new pump until both the child and pump process has died
        // The resons why we don't just waint for the child to die before spawning a daemons is:
        //  in case the child output something a SIGPIPE will be send to the process and it will die
        //  before a new daemon was spawned.
        var daemon = core.spawnDaemon(options, [child.pid, process.pid]);
