...
(function () {
  ...
  var Monitor;
  if (options.monitor) {
    Monitor = require(options.monitor).Monitor;
  }
  ...
  var setupMonitor = function (emits, message, daemonPid) {
    // do not start monitor if none exist, just spawn child
    if (options.monitor === null) {
      child.spawn();
      return;
    }

    var monitor = new Monitor(message, options, daemonPid, shutdown, function () {
  ...
  if (options.strategy === 'daemon') {
    ...
    ipc.on('message', function (msg) {
      ...
      var stopWatcer = helpers.processWatcher(msg.daemon, function () {
        // kill child process
        child.kill();
      ...
      shutdown.daemon = function (callback) {
        ...
        helpers.processWatcher(msg.daemon, callback);
