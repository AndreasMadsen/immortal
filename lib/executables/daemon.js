...
(function () {
  ...
  var state = isRestart ? 'restart' : 'start';
  ...
  // in case of a restart we will wait until pump and child is dead
  if (isRestart) {
    var progress = new helpers.ProgressTracker(child.spawn.bind(child));
    progress.add('pump', 'child');

    // wait until pump is dead
    helpers.processWatcher(parseInt(process.argv[2], 10), progress.set.bind(progress, 'pump'));

    // wait until child is dead
    helpers.processWatcher(parseInt(process.argv[3], 10), progress.set.bind(progress, 'chuld'));
  } else {
    child.spawn();
  }
