...
(function () {
  ...
  child.stderr.on('data', function (chunk) {
    // restart process and ignore stderr chunk if the errorBuffer overflows
    if (errorBuffer.length + chunk.length >= options.bufferLimit) {
      child.kill();
      return;
    }
    errorBuffer += chunk;
  ...
  child.on('stop', function () {
    ...
    var watch = new helpers.processWatcher(processPID);
